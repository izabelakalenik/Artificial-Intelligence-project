...
def visualize_kmeans_color_predictions(imgs, accuracy_scores, predicted_labels, ground_truth_labels, topN):
    ...
    for i, (img, score, pred, gt) in enumerate(zip(imgs[:topN], accuracy_scores[:topN], predicted_labels[:topN], ground_truth_labels[:topN])):
        plt.subplot(topN // 4, 4, i + 1)
        plt.imshow(img)
...
def kmean_statistics(images, ground_truth_labels, Kmax, options=None):  
    ...
    for k in range(2, Kmax + 1):
        ...
        overall_f1_score = np.mean([score for score in accuracy_scores])  # Average F1 score across all images for this k
...
if __name__ == '__main__':
    ...
    kmean_statistics(cropped_images, ground_truth_labels, Kmax=10, options=options)
